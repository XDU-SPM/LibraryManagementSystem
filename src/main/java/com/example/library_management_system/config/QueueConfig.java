...
public class QueueConfig
{
    ...
    Queue delayQueuePerMessageTTL()
    {
        return QueueBuilder.durable(DELAY_QUEUE_PER_MESSAGE_TTL_NAME)
                .withArgument("x-dead-letter-exchange", DELAY_EXCHANGE_NAME)            // 声明了队列里的死信转发到的 DLX 名称
                .withArgument("x-dead-letter-routing-key", DELAY_PROCESS_QUEUE_NAME)    // 声明了这些死信在转发时携带的 routing-key 名称
                .build();
    }
    ...
    /**
     * 将该 DLX 绑定到实际消费队列即 delay_process_queue 上。这样所有的死信都会通过 DLX 被转发到 delay_process_queue
     *
